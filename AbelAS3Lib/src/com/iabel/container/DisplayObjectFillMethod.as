package com.iabel.container
{
	...
	public class DisplayObjectFillMethod extends AbstractFillMethod
	{
		public function DisplayObjectFillMethod()
		{
			super();
		}
		protected override function fill(container:Sprite, size:Size, item:DisplayObject):void
		{
			if(item is Bitmap) throw new ArgumentError("argument error! \"item\" must be an instance of DisplayObject!");
			...
			for(var i:uint = curColumn; i < column; i++){
				...
				for(var j:uint = curRow; j < row; j++){
					...
					var o:Class = item["constructor"];
					var oo:DisplayObject = new o();
